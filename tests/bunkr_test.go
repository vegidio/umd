...
func TestBunkr_QueryVideo(t *testing.T) {
	...
	extractor, _ := umd.New().FindExtractor("https://bunkr.cr/v/0h44nakg3gkt0wh1nb05g_source-ARUJIdg7.mp4")
	resp, _ := extractor.QueryMedia(99999, nil, true)
	err := resp.Error()

	assert.NoError(t, err)
	assert.Equal(t, NumberOfPosts, len(resp.Media))
	assert.Equal(t, "0h44nakg3gkt0wh1nb05g_source.mp4", resp.Media[0].Metadata["name"])
}
